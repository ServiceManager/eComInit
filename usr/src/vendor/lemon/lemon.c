...
*/
static void
lemon_addtext (char * zBuf,      /* The buffer to which text is added */
...
{
    ap = (struct action *)msort (
        (char *)ap, (char **)&ap->next,
    ...
    do
    {
        ...
        for (rp = lemp->rule; rp; rp = rp->next)
        {
            ...
            for (i = 0; i < rp->nrhs; i++)
            {
                ...
                {
                    if (s1->lambda == LEMON_FALSE)
                        break;
                }
                else
                {
                    progress += SetUnion (s1->firstset, s2->firstset);
                    if (s2->lambda == LEMON_FALSE)
                        break;
                }
            }
        }
...
void Configlist_sort (void)
{
    current = (struct config *)msort ((char *)current,
                                      (char **)&(current->next), Configcmp);
    currentend = 0;
    return;
}
...
int main (int argc, char ** argv)
{
    ...
    static struct s_options options[] = {
        ...
        {OPT_FSTR, "I", 0,
         "Ignored.  (Placeholder for '-I' compiler options.)"},
        {OPT_FLAG, "m", (char *)&mhflag,
         "Output a makeheaders compatible file."},
        {OPT_FLAG, "l", (char *)&nolinenosflag,
         "Do not print #line statements."},
        {OPT_FSTR, "O", 0,
         "Ignored.  (Placeholder for '-O' compiler options.)"},
...
static char * merge (char * a, char * b,
                     int (*cmp) (const char *, const char *), int offset)
{
    char *ptr, *head;
...
static int handleflags (int i, FILE * err)
{
    ...
    {
        if (strncmp (&g_argv[i][1], op[j].label, lemonStrlen (op[j].label)) ==
            0)
            break;
    }
    v = g_argv[i][0] == '-' ? 1 : 0;
    ...
    for (i = 0; op[i].label; i++)
    {
        ...
        case OPT_FINT:
            fprintf (errstream, "  -%s<integer>%*s  %s\n", op[i].label,
                     (int)(max - lemonStrlen (op[i].label) - 9), "",
                     op[i].message);
            break;
        case OPT_DBL:
        case OPT_FDBL:
            fprintf (errstream, "  -%s<real>%*s  %s\n", op[i].label,
                     (int)(max - lemonStrlen (op[i].label) - 6), "",
                     op[i].message);
            break;
        case OPT_STR:
        case OPT_FSTR:
            fprintf (errstream, "  -%s<string>%*s  %s\n", op[i].label,
                     (int)(max - lemonStrlen (op[i].label) - 8), "",
                     op[i].message);
            break;
        }
    }
...
to follow the previous rule.");
    ...
    case IN_RHS:
        ...
        {
            struct rule * rp;
            rp = (struct rule *)calloc (
                sizeof (struct rule) + sizeof (struct symbol *) * psp->nrhs +
    ...
    case WAITING_FOR_DECL_KEYWORD:
        if (ISALPHA (x[0]))
        {
            ...
            else if (strcmp (x, "token_destructor") == 0)
            {
                ...
                static char * tmp;
                ErrorMsg (psp->filename, psp->tokenlineno,
                          "%%token_destructor is deprecated in lemon--.  Use "
                          "object destructors instead.");
                psp->declargslot = &tmp;
#else
            ...
            else if (strcmp (x, "default_destructor") == 0)
            {
                ...
                static char * tmp;
                ErrorMsg (psp->filename, psp->tokenlineno,
                          "%%default_destructor is deprecated in lemon--.  Use "
                          "object destructors instead.");
                psp->declargslot = &tmp;
#else
...
#ifdef LEMONPLUSPLUS
                ErrorMsg (psp->filename, psp->tokenlineno,
                          "%%destructor is deprecated in lemon--.  Use object "
                          "destructors instead.");
    ...
    for (i = 0; z[i]; i++)
    {
                 ...
                 (strncmp (&z[i], "%ifndef", 7) == 0 && ISSPACE (z[i + 7])))
        {
            ...
            else
            {
                ...
                for (k = 0; k < nDefine; k++)
                {
                    if (strncmp (azDefine[k], &z[j], n) == 0 &&
                        lemonStrlen (azDefine[k]) == n)
                    {
                        exclude = 0;
                        break;
                    }
...
)
{
    ...
    case SHIFT:
    {
        ...
        fprintf (fp, "%*s shift        %-7d", indent, ap->sp->name,
                 stp->statenum);
        break;
    }
    case REDUCE:
...
void ReportOutput (struct lemon * lemp)
{
    ...
    for (rp = lemp->rule; rp; rp = rp->next)
    {
        ...
        if (rp->precsym)
        {
            fprintf (fp, " [%s precedence=%d]", rp->precsym->name,
                     rp->precsym->prec);
        }
        fprintf (fp, "\n");
    }
...
PRIVATE FILE * tplt_open (struct lemon * lemp)
{
    ...
    if (cp)
    {
        lemon_sprintf (buf, "%.*s.lt", (int)(cp - lemp->filename),
                       lemp->filename);
    }
    else
    {
...
PRIVATE int translate_code (struct lemon * lemp, struct rule * rp)
{
    ...
    /*
     * lemon++ -- slight modifications to lhs direct logic.
     * 1. if no lhs alias, put constructor at end so it doesn't clobber RHS
     * variable. 2. no -overwrites- comments.
     */
        ...
        ** direct writing is allowed */
        ...
        if (rp->lhs->dtnum != rp->rhs[0]->dtnum)
        {
            ErrorMsg (lemp->filename, rp->ruleline,
                      ...
                      "different datatypes.",
                      rp->lhs->name, rp->lhsalias, rp->rhs[0]->name,
                      rp->rhsalias[0]);
            lemp->errorcnt++;
        }
    }
...
** the token number of X, not the value of X */
...
#else
                            append_str ("yymsp[%d].major", -1, i - rp->nrhs + 1,
                                        0);
#endif
                            cp = xp;
                            used[i] |= 2;
    ...
    for (i = 0; i < rp->nrhs; i++)
    {
        if (rp->rhsalias[i])
        {
            if (i > 0)
            {
                ...
                {
                    ErrorMsg (lemp->filename, rp->ruleline,
                              "%s(%s) has the same label as the LHS but is not "
                              "the left-most "
                              "symbol on the RHS.",
                ...
                for (j = 0; j < i; j++)
                {
                    ...
                    {
                        ErrorMsg (lemp->filename, rp->ruleline,
                                  "Label %s used for multiple symbols on the "
                                  "RHS of a rule.",
                                  rp->rhsalias[i]);
                        lemp->errorcnt++;
                        break;
    ...
    ** saved LHS value now. */
    if (lhsdirect == 0)
    {
        append_str ("  yymsp[%d].minor.yy%d = ", 0, 1 - rp->nrhs,
                    rp->lhs->dtnum);
        append_str (zLhs, 0, 0, 0);
        append_str (";\n", 0, 0, 0);
    }
    ...
    if (rp->lhsalias && !lhsdirect)
    {
        ...
        append_str ("  yy_constructor<", 0, 0, 0);
        append_str (sp_datatype (lemp, rp->lhs), 0, 0, 0);
        append_str (">(std::addressof(yymsp[%d].minor.yy%d), std::move(", 0,
                    1 - rp->nrhs, rp->lhs->dtnum);
        append_str (rp->lhsalias, 0, 0, 0);
        append_str ("));\n", 0, 0, 0);
    }
...
)
{
    ...
    */
    ...
    lineno++;
#ifdef LEMONPLUSPLUS
    fprintf (out, "  yy_storage<%sTOKENTYPE>::type yy0;\n", name);
    lineno++;
#else
    ...
    for (i = 0; i < arraysize; i++)
    {
        if (types[i] == 0)
            continue;
#ifdef LEMONPLUSPLUS
        fprintf (out, "  yy_storage<%s>::type yy%d;\n", types[i], i + 1);
        lineno++;
#else
        fprintf (out, "  %s yy%d;\n", types[i], i + 1);
        lineno++;
...
)
{
    ...
    if (in == 0)
        return;
    out = file_open (lemp, FILE_EXTENSION, "wb");
    if (out == 0)
    {
        fclose (in);
        return;
    ...
    if (lemp->arg && lemp->arg[0])
    {
        ...
        while (i >= 1 && ISSPACE (lemp->arg[i - 1]))
            i--;
        while (i >= 1 &&
               (ISALNUM (lemp->arg[i - 1]) || lemp->arg[i - 1] == '_'))
            i--;
    ...
    if (lemp->ctx && lemp->ctx[0])
    {
        ...
        while (i >= 1 && ISSPACE (lemp->ctx[i - 1]))
            i--;
        while (i >= 1 &&
               (ISALNUM (lemp->ctx[i - 1]) || lemp->ctx[i - 1] == '_'))
            i--;
    ...
    */
    ...
    if (lemp->errsym)
    {
        ...
        if (sp->useCnt)
        {
            ...
            fprintf (
                out,
                "      yy_destructor<%s>(std::addressof(yypminor->yy%d));\n",
                sp_datatype (lemp, sp), sp->dtnum);
            lineno++;
            fprintf (out, "      break;\n");
            lineno++;
        }
    ...
    if (lemp->tokendest)
    {
        ...
        for (i = 0; i < lemp->nsymbol && lemp->symbols[i]->type != TERMINAL;
             i++)
            ;
        if (i < lemp->nsymbol)
        {
...
int CompareHeader (FILE * in, struct lemon * lemp, const char * prefix)
{
    ...
    if (lemp->header && lemp->header[0])
    {
        ...
        if (!tmp)
            return 0;
        if (fread (tmp, 1, len, in) != len)
        {
            free (tmp);
            return 0;
        }
        if (memcmp (lemp->header, tmp, len))
        {
            free (tmp);
            return 0;
        }
    ...
    */
    for (i = 0; i < lemp->nstate; i++)
    {
        ...
        for (ap = stp->ap; ap; ap = nextap)
        {
            ...
            ** apply it to terminal symbols too, but that makes the parser
            *tables * larger. */
            if (ap->sp->index < lemp->nterminal)
                continue;
#endif
...
int Strsafe_insert (const char * data)
{
    ...
    if (x1a->count >= x1a->size)
    {
        ...
        array.tbl =
            (x1node *)calloc (arrSize, sizeof (x1node) + sizeof (x1node *));
        if (array.tbl == 0)
...
int Symbol_insert (struct symbol * data, const char * key)
{
    ...
    if (x2a->count >= x2a->size)
    {
        ...
        array.tbl =
            (x2node *)calloc (arrSize, sizeof (x2node) + sizeof (x2node *));
        if (array.tbl == 0)
            return 0; /* Fail due to malloc failure */
...
int State_insert (struct state * data, struct config * key)
{
    ...
    if (x3a->count >= x3a->size)
    {
        ...
        array.tbl =
            (x3node *)calloc (arrSize, sizeof (x3node) + sizeof (x3node *));
        if (array.tbl == 0)
            return 0; /* Fail due to malloc failure */
...
int Configtable_insert (struct config * data)
{
    ...
    if (x4a->count >= x4a->size)
    {
        ...
        array.tbl =
            (x4node *)calloc (arrSize, sizeof (x4node) + sizeof (x4node *));
        if (array.tbl == 0)
            return 0; /* Fail due to malloc failure */
